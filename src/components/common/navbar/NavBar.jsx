...
const menu = navItems.map((item) => (
  <li key={item.id} onMouseDown={(e) => e.preventDefault()}>
    <Link
      ...
      spy={true}
      offset={-140}
      className={`link-navbar px-5 py-3 mx-1`}
      style={{color: 'white'}}
    >
      {item.name}
    </Link>
...
const NavBar = () => {
  ...
  return (
    <div
      ...
      style={{backgroundColor: 'oklch(25.33% 0.016 252.42)'}}
    >
      <div className="navbar flex justify-between mx-auto content">
        <div className="flex items-center justify-between">
          <div className="dropdown">
            ...
            <ul
              tabIndex={0}
              className={`menu menu-lg dropdown-content rounded-box z-1 mt-3 w-lvw p-2 shadow font-semibold flex-nowrap bg-black text-black`}
            >
              {menu}
            </ul>
          </div>
